...
class MessengerDocsApp:
    ...
    def process_log_queue(self):
        try:
            ...
            if hasattr(self, 'root') and self.root.winfo_exists(): self.root.after(100, self.process_log_queue)
    ...
    def stop_monitoring(self):
        if self.is_monitoring and self.monitoring_thread and self.monitoring_thread.is_alive():
            self.log("감시 중지 요청..."); self.stop_event.set(); self.stop_button.configure(state="disabled")
        else: self.log("현재 감시 중 아님."); self.on_monitoring_stopped()
