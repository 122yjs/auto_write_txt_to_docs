...
CACHE_FILE = "added_lines_cache.json" # 라인 캐시 파일명
    ...
    for enc in encodings:
        try:
            with open(filepath, 'r', encoding=enc) as f:
                ...
                content = f.read()
            break # 읽기 성공 시 종료
        except (UnicodeDecodeError, FileNotFoundError, OSError, Exception):
...
def process_file(filepath, config, services, log_func):
    ...
    try:
        ...
        if not truly_new_lines: # 추가할 새 라인 없음
            # log_func("  - 라인 캐시 비교 후 추가할 내용 없음.") # 로그 간소화
            processed_file_states.setdefault(filepath, {})['size'] = current_size
            processed_file_states[filepath]['timestamp'] = current_time
            return
        ...
        if docs_update_successful:
            # log_func(f"  - 라인 캐시에 새로운 {len(truly_new_lines)}줄 추가...") # 로그 간소화
...
def run_monitoring(config, log_func_threadsafe, stop_event):
    ...
    if get_google_services and credentials_path and os.path.exists(credentials_path):
        try:
            ...
            else: log_func_threadsafe("경고: Google 서비스 로드 실패.")
        except Exception as e: log_func_threadsafe(f"오류: Google 서비스 초기화 예외 - {e}")
    ...
    try:
        ...
        except Exception as e: log_func_threadsafe(f"경고: Observer 스레드 join 중 오류 - {e}")
